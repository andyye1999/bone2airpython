...
from util.utils import sample_fixed_length_data_aligned,frame,pad
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == "train":
            ...
            # if len(mixture) < 8192:
            #     mixture = pad(mixture, 8192)
            batches = int((len(mixture) - 4096) / 4096)
            mixture = mixture[0: int(batches * 4096 + 4096)]
            mixture1 = mixture.copy()
            # if len(clean) < 8192:
            #     clean = pad(clean, 8192)
            batches1 = int((len(clean) - 4096) / 4096)
            clean = clean[0: int(batches1 * 4096 + 4096)]
            clean1 = clean.copy()
            X = frame(mixture1, 8192, 4096)[:, np.newaxis, :]

            y = frame(clean1, 8192, 4096)[:, np.newaxis, :]
            return torch.tensor(X), torch.tensor(y)
        else:
            return mixture, clean, filename
