...
from util.utils import sample_fixed_length_data_aligned,normalize_data,sample_fixed,copymag4
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        mixture = copymag4(mixture)
        clean, _ = librosa.load(os.path.abspath(os.path.expanduser(clean_path)), sr=16000)
        mixture, clean, max_bone, max_air = normalize_data(mixture, clean)
        if self.mode == "train":
            ...
            return mixture.reshape(1, -1), clean.reshape(1, -1), filename, max_bone, max_air
        else:
            return mixture.reshape(1, -1), clean.reshape(1, -1), filename, max_bone, max_air
