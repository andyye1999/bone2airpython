...
class COPYBLSTMModel(nn.Module):
    def __init__(self):
        ...
        self.lstm1 = nn.LSTM(input_size=51, hidden_size=161, num_layers=3, dropout=0.02, bidirectional=True,
                             batch_first=True)
    ...
    def forward(self, X):
        ...
        # pred_stft_real[:,49:65,:] = pred_stft_real[:,33:49,:]
        # pred_stft_real[:, 65:81, :] = pred_stft_real[:,33:49,:]
        # pred_stft_real[:, 81:97, :] = pred_stft_real[:,33:49,:]
        # pred_stft_real[:, 97:113, :] = pred_stft_real[:,33:49,:]
        # pred_stft_real[:, 113:129, :] = pred_stft_real[:,33:49,:]
        # pred_stft_real[:, 129:145, :] = pred_stft_real[:,33:49,:]
        # pred_stft_real[:, 145:161, :] = pred_stft_real[:,33:49,:]
        # pred_stft_imag[:, 49:65, :] = pred_stft_imag[:, 33:49, :]
        # pred_stft_imag[:, 65:81, :] = pred_stft_imag[:, 33:49, :]
        # pred_stft_imag[:, 81:97, :] = pred_stft_imag[:, 33:49, :]
        # pred_stft_imag[:, 97:113, :] = pred_stft_imag[:, 33:49, :]
        # pred_stft_imag[:, 113:129, :] = pred_stft_imag[:, 33:49, :]
        # pred_stft_imag[:, 129:145, :] = pred_stft_imag[:, 33:49, :]
        # pred_stft_imag[:, 145:161, :] = pred_stft_imag[:, 33:49, :]
        pred_mag = torch.sqrt(pred_stft_real ** 2 + pred_stft_imag ** 2 + 1e-6)
        low_mag = pred_mag[:,0:51,:]
        # pred_mag[:, 65:81, :] = pred_mag[:,33:49,:]
        # pred_mag[:, 81:97, :] = pred_mag[:,33:49,:]
        # pred_mag[:, 97:113, :] = pred_mag[:,33:49,:]
        # pred_mag[:, 113:129, :] = pred_mag[:,33:49,:]
        # pred_mag[:, 129:145, :] = pred_mag[:,33:49,:]
        # pred_mag[:, 145:161, :] = pred_mag[:,33:49,:]
        ...
        # print(out.shape)
        input = low_mag.permute(0, 2, 1)  # 指定维度新的位置 (Bs, T, F)
...
class BLSTM(nn.Module):
    ...
    def __init__(self):
        ...
        hidden_size = 161
        num_layers = 1
        self.blstm = nn.LSTM(input_size=51, hidden_size=hidden_size, num_layers=num_layers, bidirectional=True, batch_first=True)
...
class CRN(nn.Module):
    ...
    def forward(self, X):
        ...
        # batch_size, m, z = X.shape  # (batch,1,16320)
        # newshape = (batch_size, z)
        ...
        # pred_mag = torch.sqrt(pred_stft_real ** 2 + pred_stft_imag ** 2 + 1e-6)
        # pred_mag[:, 41:81, :] = pred_mag[:, 1:41, :]
        # pred_mag[:, 81:121, :] = pred_mag[:, 1:41, :]
        # pred_mag[:, 121:161, :] = pred_mag[:, 1:41, :]
        # if torch.isnan(pred_stft).any():
        #     print("nan error")
        #     input("press enter pred_stft nan error")
        # if torch.isnan(pred_stft_real).any():
        ...
        # print(out.shape)
        # input = pred_mag.permute(0, 2, 1)  # 指定维度新的位置 (Bs, T, F)
        # input = pred_mag  # 指定维度新的位置 (Bs, F, T)

        # h1 = torch.randn(3*2, batch_size, 161).to(device)  # [num_layers(=3) * num_directions(=2), batch_size, n_hidden]
        # c1 = torch.randn(3*2, batch_size, 161).to(device)
        X = X.permute(0, 2, 1)  # 指定维度新的位置 (Bs, T, F)
        out = self.crn(X)
        # out, (_, _) = self.lstm1(input)
        # out = self.fc(out)
        # out = self.relu(out)
