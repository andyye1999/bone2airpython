...
import torch
from model.utils.conv_stft import *
from model.utils.complexnn import *
...
class DCCRN(nn.Module):
    def __init__(self,
                 ...
                 use_clstm=True, use_cbn=False, masking_mode='mapping',
...
class DCCRN256(nn.Module):
    def __init__(self,
                 rnn_layer=2, rnn_hidden=128,
                 win_len=200, hop_len=100, fft_len=256, win_type='hanning',
                 use_clstm=True, use_cbn=False, masking_mode='mapping',
                 kernel_size=5, kernel_num=(32, 64, 128, 256, 256, 256)
                 ):
        super(DCCRN256, self).__init__()
        self.rnn_layer = rnn_layer
        self.rnn_hidden = rnn_hidden
...
if __name__ == '__main__':
    ...
    test_model = DCCRN()
    print(test_model)
    x = torch.randn(16,16000*2)
    t = torch.randn(16,16000*2)
    out_real, target_real, out_imag, target_imag, out_wav = test_model(x,t)
    print(out_real.shape)
    print(target_real.shape)
    print(out_imag.shape)
    print(target_imag.shape)
    print(out_wav.shape)
