...
class Model(nn.Module):
    ...
    def forward(self, input):
        ...
        pred_stft = torch.stft(X, n_fft=256, hop_length=128, win_length=256)  # (Bs, F, T, 2)
        pred_stft_real, pred_stft_imag = pred_stft[:, :, :, 0], pred_stft[:, :, :, 1]
        pred_stft_real[:, 28:38, :] = pred_stft_real[:, 58:68, :]
        pred_stft_imag[:, 28:38, :] = pred_stft_imag[:, 58:68, :]
        pred_mag = torch.sqrt(pred_stft_real ** 2 + pred_stft_imag ** 2 + 1e-6)
        mag = pred_mag
        # mag = torch.exp(mag) - 1e-12  # 将对数幅度谱转换成幅度谱
        pha = torch.atan2(pred_stft_imag, pred_stft_real)  # ([16, 161, 103])
        real = mag * torch.cos(pha)
        imag = mag * torch.sin(pha)
        model = torch.istft(torch.cat([real.unsqueeze(-1), imag.unsqueeze(-1)], dim=-1), n_fft=256, hop_length=128,
                            win_length=256)
        input = torch.unsqueeze(model, 1)
        o = input
