...
def main(args):
    ...
    for dir in tqdm(clean_list):
        ...
        idx += 1
        if idx == 50:  # stack audios into one buffer and split
            split_list = np.array_split(buffer[:(len(buffer) - len(buffer) % (int(len_s * sr)))],
                                        (len(buffer) - len(buffer) % (int(len_s * sr))) // (int(len_s * sr)))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--raw_clean_dir", default="F:\\yhc\\bone\\Bone_Air_LCH\\air\\train",
                        help='Path to the dir containing raw clean speech audio clips to be splited')
    parser.add_argument("--saved_clean_dir", default="F:\\yhc\\bone\\Bone_Air_LCH\\air\\train2s",
                        help='Path to the dir saving splited clean speech audio clips')
    parser.add_argument("--raw_noise_dir", default="F:\\yhc\\bone\\Bone_Air_LCH\\bone\\train",
                        help='Path to the dir containing raw noise audio clips to be splited')
    parser.add_argument("--saved_noise_dir", default="F:\\yhc\\bone\\Bone_Air_LCH\\bone\\train2s",
                        help='Path to the dir saving splited noise audio clips')
    parser.add_argument("--segment_length", default=2,
