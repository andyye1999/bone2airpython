...
from model.soundstream import SoundStream1
...
class BaseTrainer:
    def __init__(self,
        ...
        self.model = SoundStream1(32, 32, 6, 1024, [4, 4, 5, 10]).to(self.device)
        ...
        self.loss1 = cntloss()
        self.loss2 = SpectralReconstructionLoss(sr=8000, reduction='mean', device=self.device)
        # self.model = model.to(self.device)
        # self.model = self.generator
        self.optimizer = torch.optim.Adam(params=self.model.parameters(), lr=0.0001, betas=(0.5, 0.9))
        self.optimizer2 = torch.optim.Adam(params=self.discriminator.parameters(), lr=0.0001, betas=(0.5, 0.9))
