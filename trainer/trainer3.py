...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for i, (mixture, clean, name, _, _) in enumerate(self.train_data_loader):
            ...
            out_real, target_real, out_imag, target_imag, out_wav = self.model(input,target)
            # real_loss = self.loss_function(out_real, target_real)
            # imag_loss = self.loss_function(out_imag, target_imag)
            # out_mag = torch.sqrt(out_real ** 2 + out_imag ** 2 + 1e-6)
            # target_mag = torch.sqrt(target_real ** 2 + target_imag ** 2 + 1e-6)
            # mag_loss = self.loss_function(out_mag, target_mag)
            # loss = (real_loss + imag_loss + mag_loss) / 3
            loss = self.loss_function(out_wav,target)
            loss.backward()
            self.optimizer.step()
    ...
    def _validation_epoch(self, epoch):
        ...
        for i, (mixture, clean, name, max_bone, max_air) in enumerate(self.validation_data_loader):
            ...
            for chunk in mixture_chunks:
                ...
                # pred_mag = torch.log(pred_mag ** 2 + 1e-6)  # 将计算幅度谱改成计算对数幅度谱
                # input = pred_mag.permute(0, 2, 1)  # 指定维度新的位置 (Bs, T, F)
                out_real, target_real, out_imag, target_imag, out_wav = self.model(chunk,tmp)
