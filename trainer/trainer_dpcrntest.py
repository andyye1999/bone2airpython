...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        imagloss_total = 0.0
        magloss_total = 0.0
        for i, (mixture, clean, name, _, _) in enumerate(self.train_data_loader):
            ...
            # input = line(input) # disaplay line
            ...
            input_stft = torch.stft(input, n_fft=400, hop_length=200, win_length=400)  # (Bs, F, T, 2)

            output_stft = self.model(input_stft)
            out_wav = torch.istft(output_stft, n_fft=400,
                                  hop_length=200,
                                  win_length=400)
            # real_loss = self.loss_function(output_stft[:, :, :, 0], input_stft[:, :, :, 0])
            # imag_loss = self.loss_function(output_stft[:, :, :, 1], input_stft[:, :, :, 1])
            loss,realloss,imagloss,magloss = self.loss_function(out_wav,target)
            loss.backward()
            self.optimizer.step()

            loss_total += loss.item()
            realloss_total += realloss.item()
            imagloss_total += imagloss.item()
            magloss_total += magloss.item()

        self.schedular.step()
        dl_len = len(self.train_data_loader)
        self.writer.add_scalar(f"Train/Loss", loss_total / dl_len, epoch)
        self.writer.add_scalar(f"Train/realloss", realloss_total / dl_len, epoch)
        self.writer.add_scalar(f"Train/imagloss", imagloss_total / dl_len, epoch)
        self.writer.add_scalar(f"Train/magloss", magloss_total / dl_len, epoch)
    ...
    def _validation_epoch(self, epoch):
        ...
        for i, (mixture, clean, name, max_bone, max_air) in enumerate(self.validation_data_loader):
            ...
            clean = clean.to(self.device)  # [1, 1, T]
            ...
            for chunk in mixture_chunks:
                ...
                # chunk = line(chunk) # disaplay line
                tmp = torch.squeeze(chunk, dim=1)
                # pred_stft = torch.stft(bone_normalized, n_fft=320, hop_length=160, win_length=320)  # (Bs, F, T, 2)
                # pred_stft_real, pred_stft_imag = pred_stft[:, :, :, 0], pred_stft[:, :, :, 1]
                # pred_stft_real[:, 38:48, :] = pred_stft_real[:, 101:111, :]
                # pred_stft_real[:, 38:48, :] = pred_stft_real[:, 101:111, :]
                ...
                input_stft = torch.stft(chunk, n_fft=400, hop_length=200, win_length=400)  # (Bs, F, T, 2)
